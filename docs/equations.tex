...
% \xspace matters here. TeX discards the space that follows a control word, so
% "\established\nWith base values" renders as "[E]With". These tags are used 46
% times, mostly followed by running text on the next source line, so fixing it
% at the macro rather than at each call site is the only maintainable option.
% \xspace suppresses itself before punctuation and braces, so "\unvalidated{}"
% and "\established," still come out right.
\newcommand{\established}{\textcolor{okgreen}{\textbf{[E]}}\xspace}
\newcommand{\approxi}{\textcolor{amber}{\textbf{[A]}}\xspace}
\newcommand{\unvalidated}{\textcolor{danger}{\textbf{[U]}}\xspace}
\newcommand{\src}[1]{\hfill{\footnotesize\texttt{#1}}}

\pagestyle{fancy}
\fancyhf{}
\lhead{\footnotesize Goddard 26--27 --- Model Equations --- v\docversion}
\rhead{\footnotesize\thepage}
\renewcommand{\headrulewidth}{0.4pt}

% ------------------------------------------------------------ document version
% Deliberately versioned rather than dated: a date says when the file was
% typeset, which tells a reader nothing about whether the physics changed.
% Bump this line whenever an equation changes.
\newcommand{\docversion}{test}
...
\medskip
% p-column sized off \linewidth (the minipage width, not the page width) so the
% descriptions wrap instead of protruding out of the surrounding \fbox. A plain
% two-column tabular cannot wrap and overflows silently -- LaTeX issues no
% Overfull warning for a tabular that is simply wider than its minipage.
\begin{tabular}{@{}l@{\hspace{0.7em}}p{\dimexpr\linewidth-3.4em\relax}@{}}
\established & Established result. Analytic, or a long-standing published
               correlation, implemented directly. \\[3pt]
\approxi     & Engineering approximation. Right form and magnitude; the
               leading constants are calibrated, not derived. \\[3pt]
...
\paragraph{Nose-to-body transition: none.}
The source OpenRocket file carried a sub-body-diameter nose flaring out to the
body tube. A flare is a supersonic wave-drag source with no aerodynamic return
on this vehicle, and it has been \textbf{removed} (register B7): the nose meets
the body at full diameter, so $D_{nose,base} = D_{body} = \SI{0.1524}{m}$ and
there is no transition term in either the drag build-up or the normal-force
sum.

The transition machinery in \texttt{aero/geometry.py},
\texttt{aero/drag.py} and \texttt{aero/normal\_force.py} is retained rather
than deleted. It is implemented and tested, every downstream term is guarded on
a \texttt{present} flag and contributes exactly zero at zero length, and
keeping it means reversing the decision is a configuration change rather than a
rewrite. Its equations are omitted here because they are not part of the
current design.
...
A single canopy in two states: deployed reefed at apogee, then disreefed
to full. There is no drogue -- the reefed stage does that job, which is the
whole reason a separate drogue is unnecessary.

\paragraph{Reefed drag area.} \established
Drag area scales with the square of the diameter ratio:
\begin{equation}
(C_dS)_{reefed} = (C_dS)_{canopy}\cdot \Lambda^2
\end{equation}
